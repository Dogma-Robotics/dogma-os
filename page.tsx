...
// ── Agents ──
var AGENTS=[
{id:"planner",name:"Planner",icon:"📐",color:C.gold,cat:"strategy",desc:"Implementation planner — phased blueprints, milestones, critical path"},
...
{id:"backend-dev",name:"Backend Dev",icon:"⚙️",color:C.g,cat:"build",desc:"Senior backend — TDD, APIs, validation, error handling"},
{id:"frontend-dev",name:"Frontend Dev",icon:"🎨",color:C.a,cat:"build",desc:"Senior frontend — React, Next.js, Tailwind, accessibility"},
{id:"coder",name:"Coder",icon:"💻",color:C.g,cat:"build",desc:"General-purpose coder — any language, TDD-strict, production-grade"},
{id:"data-engineer",name:"Data Engineer",icon:"🗄️",color:C.c,cat:"build",desc:"Data engineer — schemas, migrations, queries, ETL"},
{id:"mobile-dev",name:"Mobile Dev",icon:"📱",color:C.b,cat:"build",desc:"Mobile dev — React Native, Swift, Kotlin, offline-first"},
{id:"reviewer",name:"Code Reviewer",icon:"🔎",color:C.a,cat:"quality",desc:"Code reviewer — correctness, security, performance scoring"},
...
{id:"perf-benchmarker",name:"Performance",icon:"📊",color:C.gold,cat:"quality",desc:"Performance — profiling, benchmarks, latency optimization"},
{id:"e2e-runner",name:"E2E Tester",icon:"🎭",color:C.r,cat:"quality",desc:"E2E test engineer — Playwright/Cypress, user journey testing"},
{id:"build-resolver",name:"Build Fixer",icon:"🔧",color:C.r,cat:"ops",desc:"Build error specialist — compilation, bundling, deployment fixes"},
{id:"refactor-cleaner",name:"Refactorer",icon:"🧹",color:C.g,cat:"ops",desc:"Code cleanliness — dead code removal, complexity reduction"},
{id:"documenter",name:"Documenter",icon:"📚",color:C.b,cat:"ops",desc:"Documentation — READMEs, API docs, runbooks, changelogs"},
{id:"cicd-engineer",name:"CI/CD",icon:"🔄",color:C.g,cat:"ops",desc:"CI/CD engineer — pipelines, deployment, monitoring, rollback"},
{id:"optimizer",name:"Optimizer",icon:"⚡",color:C.gold,cat:"ops",desc:"Cost & perf optimizer — compute, tokens, infra savings"},
{id:"coordinator",name:"Coordinator",icon:"🎯",color:C.gold,cat:"ops",desc:"Swarm coordinator — synthesis, conflict resolution, summary"},
];
var AGENT_CATS=[{id:"strategy",label:"Strategy",color:C.gold},{id:"build",label:"Build",color:C.g},{id:"quality",label:"Quality",color:C.r},{id:"ops",label:"Ops",color:C.b}];
...
var sendAgent=function(){if(!inp.trim()||ld)return;var userTxt=inp.trim();setInp("");
  ...
  .then(function(data){
    ...
    if(mutations.length>0){fetch("/api/data").then(function(r){return r.json();}).then(function(fresh){if(fresh&&fresh.data)setD(fresh.data);}).catch(function(){});}
  })
  .catch(function(err){
  ...
  <div style={{flex:1,display:"flex",overflow:"hidden",position:"relative"}}>
    ...
    <div style={{width:chatW,minWidth:240,borderLeft:"1px solid "+C.bd,background:C.bg1,display:"flex",flexShrink:0,position:"relative"}}>
      ...
      <div style={{padding:"6px 8px",borderBottom:"1px solid "+C.bd,flexShrink:0,maxHeight:160,overflowY:"auto"}}>
        {AGENT_CATS.map(function(cat){var catAgents=AGENTS.filter(function(a){return a.cat===cat.id;});return <div key={cat.id} style={{marginBottom:3}}>
      ...
      {/* Objective input */}
      <div style={{padding:"8px 10px",borderBottom:"1px solid "+C.bd,flexShrink:0}}>
        <div style={{fontSize:10,color:C.tx3,textTransform:"uppercase",marginBottom:4}}>20 Unified Agents</div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:10}}>
        {!swarmResult&&!swarmLoading&&<div style={{textAlign:"center",padding:20,color:C.tx3}}>
          ...
          <div style={{fontSize:12,marginTop:4}}>Select a workflow, describe your objective, and launch. Multiple specialized agents will collaborate in stages — each building on the previous agent's output.</div>
